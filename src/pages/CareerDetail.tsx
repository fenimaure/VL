        ...
        <div className="min-h-screen bg-white dark:bg-dark-950 text-gray-900 dark:text-gray-300 transition-colors duration-500">
            ...
            <section className="pt-32 pb-20">
                <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                    <Link to="/careers" className="inline-flex items-center gap-2 text-black/40 hover:text-black dark:text-gray-400 dark:hover:text-white mb-8 transition-colors">
                        <ArrowLeft className="h-4 w-4" /> Back to Careers
                    </Link>

                    <h1 className="text-4xl md:text-5xl font-bold text-black dark:text-white font-display mb-6 transition-colors duration-500">{job.title}</h1>

                    <div className="flex flex-wrap gap-4 mb-8">
                        <span className="px-3 py-1 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-full text-sm text-black dark:text-white flex items-center gap-2 transition-colors duration-500">
                            <Briefcase className="h-4 w-4 text-primary-500 dark:text-primary-400" /> {job.department}
                        </span>
                        <span className="px-3 py-1 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-full text-sm text-black dark:text-white flex items-center gap-2 transition-colors duration-500">
                            <MapPin className="h-4 w-4 text-primary-500 dark:text-primary-400" /> {job.location}
                        </span>
                        <span className="px-3 py-1 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-full text-sm text-black dark:text-white flex items-center gap-2 transition-colors duration-500">
                            <Clock className="h-4 w-4 text-primary-500 dark:text-primary-400" /> {job.type}
                        </span>
                        {job.salary_range && (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
                        <div className="md:col-span-2 space-y-12">
                            ...
                            <div>
                                <h3 className="text-xl font-bold text-black dark:text-white mb-4 transition-colors duration-500">Requirements</h3>
                                <ul className="space-y-3">
                                    {job.requirements?.map((req: string, i: number) => (
                                        <li key={i} className="flex gap-3 text-gray-700 dark:text-gray-300 transition-colors duration-500">
                            ...
                            {job.benefits && job.benefits.length > 0 && (
                                <div>
                                    <h3 className="text-xl font-bold text-black dark:text-white mb-4 transition-colors duration-500">Benefits</h3>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        {job.benefits.map((benefit: string, i: number) => (
                                            <div key={i} className="bg-black/5 dark:bg-white/5 p-4 rounded-lg border border-black/5 dark:border-white/5 text-black/80 dark:text-gray-300 transition-colors duration-500">
                                                {benefit}
                                            </div>
                                        ))}
                                    </div>
